...
const create = async (req, res) => {
  ...
  let response = await nuvemshop.post("products", payload);
...
const update = async (req, res) => {
  ...
  for (let v of variants) {
    ...
    for (let i = 1; i <= 5; i++) {
      let responseVariant = await nuvemshop.put(`products/${id}/variants/${v?.id}`, v);
  ...
  if (variantsInsert.length > 0) {
    for (let v of variantsInsert) {
      let responseVariantsInsert = await nuvemshop.post(`products/${id}/variants`, v);
